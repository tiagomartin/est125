...
"Caso precisem de variáveis adicionais, posso consultar os sistemas internos para ver o que pode ser exportado. Mas comecem com [este conjunto](). Ele cobre 99% das saídas registradas no último trimestre."
...
3) Considerando o objetivo de construir um modelo de classificação para prever o *churn*, quais duas colunas do conjunto de dados vocês consideram que provavelmente não serão úteis como *features* preditivas (ou seja, não trarão valor para o modelo)? Justifiquem sua resposta.
...
## 🧺 Fase 5: Sistema de Recomendação Baseado em Regras de Associação
