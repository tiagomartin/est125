...
$$lift = \dfrac{Conf(X \Rightarrow Y)}{Sup(Y)} = \dfrac{Sup(X \cup Y)}{Sup(X)Sup(Y)}$$
...
## Tipos de dados utilizados em Associação

Os algoritmos de associação trabalham com **itens presentes nas transações**, e não com valores numéricos diretamente.<br><br>

### Preparação dos dados

➡️ **1. Variáveis nominais**

- Utilizadas diretamente como itens.
- Ex.: Sexo = Masculino, Estado = SP, Produto = Leite.

➡️ **2. Variáveis contínuas**

- Devem ser **discretizadas** em categorias ou intervalos.
...
➡️ **3. Representação binária**

- Cada categoria é convertida em um item binário (0/1).
- Apenas a **presença (1)** é considerada na mineração.
- Essa representação é denominada **variáveis binárias assimétricas**.

> **Pré-processamento:** antes da mineração de regras, todos os dados devem ser convertidos para um **formato transacional**, composto por conjuntos de itens presentes em cada transação.
...
- **Primeira regra:** {citrus fruit, root vegetables}  ⇒   {other vegetables}

> Clientes que compram frutas cítricas e legumes de raiz também costumam comprar outros vegetais.


**Interpretação:** Das 9.855 compras, apenas 102 continham simultaneamente frutas cítricas, legumes de raiz e outros vegetais (support = 0,01037112). Entretanto, sempre que alguém comprou frutas cítricas e legumes de raiz, 58,6% das vezes também comprou outros vegetais (confidence = 0,5862069). Além disso, essa probabilidade é aproximadamente 3 vezes maior do que a observada para um consumidor qualquer (lift = 3,029608), indicando uma associação positiva entre esses produtos.
...
- **Whole milk** e **other vegetables** são os principais consequentes, aparecendo como destino de diversas regras.

- **Root vegetables** é o principal item de ligação (hub), participando de várias combinações no antecedente.

- **Yogurt** também aparece em múltiplas regras, enquanto **citrus fruit**, **butter**, **domestic eggs**, **pip fruit** e outros itens participam apenas de associações específicas.


## Exemplo de Regras de Associação com a Base de Dados `Groceries`


> **Conclusão:** a rede evidencia que **leite integral** e **outros vegetais** desempenham papel central nas cestas de compras. Essas associações podem orientar estratégias de **cross-selling**, promoções conjuntas, organização das gôndolas e sistemas de recomendação de produtos.
...
# pedido hipotetico
# Vamos definir o número de pedidos que você quer avaliar
num_pedidos <- 2 
...
# Pedido 2: Peixe, Queijo Fatiado e Leite Integral
