...
![](/images/impor_pre_process.png){width=70%}
:::
...
::: {.content-visible when-format="html"}
![[Fonte: Giphy](https://giphy.com/gifs/ndr-esc-eurovision-eurovisionde-7JHtjml53hhMgCGFgO)](https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExc2p6YjM3enR1eWZ3Y2lsMGloZ3JuMmtvMjJpcXk3cW9kZGE2NG5neCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/7JHtjml53hhMgCGFgO/giphy.gif){fig-align="center" .r-stretch}
:::

::: {.content-visible when-format="pdf"}
![](/images/ruidosos.png){width=70%}
:::
...
x <- seq(0, 10, length.out = 180)
sinal <- sin(x) + 0.15 * x
y <- sinal + rnorm(length(x), sd = 0.45)
...
ggplot(dados, aes(x = x)) +
  geom_ribbon(aes(ymin = li, ymax = ls), alpha = 0.18, fill = "#C93C3C") +
  geom_point(aes(y = y), alpha = 0.45, size = 1.7, color = "#8FB9F4") +
  geom_line(aes(y = sinal), linewidth = 1.5, color = "#C93C3C") +
  labs(
    title = "Sinal e ruído nos dados",
    subtitle = "Dados observados variam ao redor do padrão devido ao ruído",
    x = "X",
    y = "Y"
  ) +
  annotate(
    "segment",
    x = 7.6, xend = 6.8, y = 1.95, yend = 1.45,
  ...
  annotate(
    "segment",
    x = 2.2, xend = 2.2, y = 0.1, yend = -0.55,
    linewidth = 0.5, color = "grey30"
  ) +
  annotate(
    "text",
...
- O **sinal** contém a estrutura que queremos aprender  
- O **ruído** dificulta a identificação dessa estrutura  
- Um modelo ruim pode confundir ruído com padrão  

::: {.callout-important appearance="simple"}
**Overfitting ocorre quando o modelo aprende o ruído em vez do sinal.**
:::
...
Como lidar com variações indesejadas nos dados? <br><br>



::: {.callout-important appearance="simple"}
O objetivo não é eliminar o ruído, mas reduzir seu impacto sem perder o sinal.
:::
...
Valores muito distantes do padrão dos dados<br><br>
...
Valores atípicos podem ser **erro** ou **informação relevante**.
:::
...
# -----------------------------
# Dados simulados (com tendência + ruído)
# -----------------------------
x <- 1:200
sinal <- sin(x/10) + 0.05 * x
y <- sinal + rnorm(length(x), sd = 0.6)
...
# Média móvel
dados$ma <- zoo::rollmean(dados$y, k = 7, fill = NA)

# LOESS
fit_loess <- loess(y ~ x, data = dados, span = 0.2)
dados$loess <- predict(fit_loess)

# Suavização exponencial
fit_exp <- forecast::ets(dados$y)
dados$exp <- fitted(fit_exp)
...
ggplot(dados, aes(x, y)) +
  ...
  # métodos
  geom_line(aes(y = ma, color = "Média móvel"), linewidth = 1) +
  geom_line(aes(y = loess, color = "LOESS"), linewidth = 1.2) +
  geom_line(aes(y = exp, color = "Exponencial"), linewidth = 1.2) +
  ...
  labs(
    title = "Comparação de métodos de suavização",
    subtitle = "Cada método reduz o ruído de forma diferente",
    x = "Tempo / Observação",
    y = "Valor",
    color = "Método"
  ) +
...
**Seleção baseada em wrapper:** Essa técnica envolve a seleção de atributos por meio de um modelo de aprendizado de máquina, que é treinado em um conjunto de atributos selecionados. O modelo é avaliado usando validação cruzada e as atributos são selecionados com base no desempenho do modelo.
  ...
  - **Métodos wrapper:** Forward stepwise selection, Backward Elimination.
...
p1 <- ggplot(df, aes(x = x, y = after_stat(density))) +
  geom_histogram(fill = "#2C7FB8", color = "white", bins = 30) +
  labs(title = "Antes", x = "Atributo assimétrico", y = NULL) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))

p2 <- ggplot(df, aes(x = log_x, y = after_stat(density))) +
  geom_histogram(fill = "#2C7FB8", color = "white", bins = 30) +
  labs(title = "Depois", x = "log(x)", y = NULL) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))
...
- reduz **assimetria**
- diminui a **variância**
- mantém melhor a **escala original**
...
set.seed(123)
x <- rgamma(1000, shape = 1.5, rate = 1)
...
p1 <- ggplot(df, aes(x = x, y = after_stat(density))) +
  geom_histogram(fill = "#2C7FB8", color = "white", bins = 30) +
  labs(title = "Antes", x = "Atributo", y = NULL) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))

p2 <- ggplot(df, aes(x = sqrt_x, y = after_stat(density))) +
  geom_histogram(fill = "#2C7FB8", color = "white", bins = 30) +
  labs(title = "Depois", x = "√x", y = NULL) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))
...
- reduz **assimetria**
- estabiliza a **variância**
- melhora o ajuste de modelos<br><br>
...
df <- data.frame(
  original = x,
  transformado = predict(bc)
)

p1 <- ggplot(df, aes(x = original, y = after_stat(density))) +
  geom_histogram(fill = "#2C7FB8", color = "white", bins = 30) +
  labs(title = "Antes", x = "Assimétrico", y = NULL) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))

p2 <- ggplot(df, aes(x = transformado, y = after_stat(density))) +
  geom_histogram(fill = "#2C7FB8", color = "white", bins = 30) +
  labs(title = "Depois", x = "Box-Cox", y = NULL) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))

ggsave("../../images/boxcox.png", p1 + p2, width = 8, height = 4)
```
...
<p align="center"> 
⚠️ **Restrição:** Só pode ser aplicada a dados positivos
</p>
...
- semelhante ao **Box-Cox**  
- reduz **assimetria**  
- estabiliza a **variância**<br><br>
...
x <- rgamma(1000, shape = 1, rate = 1) - 1.5  # inclui valores negativos

yj <- yeojohnson(x)

df <- data.frame(
  original = x,
  transformado = predict(yj)
)
...
p2 <- ggplot(df, aes(x = transformado, y = after_stat(density))) +
  geom_histogram(fill = "#2C7FB8", color = "white", bins = 30) +
  labs(title = "Depois", x = "Yeo-Johnson", y = NULL) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))

ggsave("../../images/yeojohnson.png", p1 + p2, width = 8, height = 4)

```


![](../../images/yeojohnson.png)
...
<p align="center"> 
🚀 **Insight:** Yeo-Johnson = Box-Cox sem restrição de positividade
</p>
